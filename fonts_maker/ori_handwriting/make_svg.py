...
def work(piclist, PIC_PATH, SVG_PATH):
    for pic in piclist:
        result = png_to_svg(PIC_PATH + pic, contiguous=True, opaque=True, keep_every_point=None)
        with open(SVG_PATH + pic[:4] + '.svg', 'w', encoding='utf-8') as make_file:
            make_file.write(result)

import time

def gen_svg(trans_path, save_path):
    PIC_LIST = os.listdir(trans_path)
    ...
    for index, piclist in enumerate(lists):
        proc = Process(target=work, args=(piclist, trans_path, save_path))
        procs.append(proc)
        proc.start()
